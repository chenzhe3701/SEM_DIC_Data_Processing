...
%%
for iE = 0:5        % 'e#' in the file name, i.e., stop/pause #  ----------------------------------------------
folderName = [fileNamePrefix_1,num2str(iE)];
...
for iR = 1:size(FOV,1)
    for iC = 1:size(FOV,2)
       fName1 = [fileNamePrefix_1,num2str(iE),fileNamePrefix_2,FOV{iR,iC}]
       I = double(imread([path_DIC,'\',folderName,'\',fName1,'.tif']));    % otherwise, nanmean doesn't work, bucause cat makes nan to 0.
...
img = uint16(img);
imwrite(img,[path_target,'\fullImage_',num2str(iE),'.tif']);
imgR = img(1:reduction:end,1:reduction:end);
imwrite(imgR,[path_target,'\reducedResolutionImage_',num2str(iE),'.tif']);
